...
//    }
	...
	public long countByRealEstateStatus(String status) {
		try {
			...
			if (!status.equals(StatusRealEstateConstant.CONFUSED)
					&& !status.equals(StatusRealEstateConstant.NOT_VERIFIED)
					&& !status.equals(StatusRealEstateConstant.VERIFIED)) {
				throw new Exception();
			}
	...
	public long countByRealEstateSourceNot(String realEstateAddress, String realEstateSource) {
		try {
			if (realEstateAddress == null || (realEstateAddress != null && realEstateAddress.isEmpty())) {
				throw new Exception();
			}
			if (realEstateSource == null || (realEstateSource != null && realEstateSource.isEmpty())) {
				throw new Exception();
			}
			RealEstateSpecifications spec1 = new RealEstateSpecifications(
					new SearchCriteria("realEstateName", ":", realEstateAddress));
