...
public class RealEstateService implements IRealEstateService {
	...
	@Override
	public List<RealEstate> listFilterRealEstate(String realEstateName, String realEstateSource,
		...
		try {
			...
			if (realEstateSource != null && !realEstateSource.isEmpty()) {
				listSpec.add(
						new RealEstateSpecifications(new SearchCriteria("realEstateSource", ":=", realEstateSource)));
			} else if (realEstateSource.isEmpty() || realEstateSource == null) {
				throw new Exception();
			}
			if (realEstateStatus != null && !realEstateStatus.isEmpty()) {
				listSpec.add(
						new RealEstateSpecifications(new SearchCriteria("realEstateStatus", ":=", realEstateStatus)));
			...
			case 3:
				return realEstateRepository
						.findAll(Specification.where(listSpec.get(0).and(listSpec.get(1).and(listSpec.get(2)))));
			default:
	...
	public Page<RealEstate> listFilterRealEstateByAddress(String realEstateAddress, Pageable page) {
		try {
			...
			RealEstateSpecifications spec1 = new RealEstateSpecifications(
					new SearchCriteria("realEstateName", ":", realEstateAddress));
			RealEstateSpecifications spec2 = new RealEstateSpecifications(
					new SearchCriteria("realEstateAddress", ":", realEstateAddress));
	...
	public long countByRealEstateSource(String realEstateAddress, String realEstateSource) {
		try {
			...
			RealEstateSpecifications spec2 = new RealEstateSpecifications(
					new SearchCriteria("realEstateAddress", ":", realEstateAddress));
			RealEstateSpecifications spec3 = new RealEstateSpecifications(
					new SearchCriteria("realEstateSource", ":=", realEstateSource));
	...
	@Override
	public Page<RealEstate> listFilterRealEstateByAddressAndSource(String realEstateAddress, String realEstateSource,
			Pageable page) {
		try {
			if (realEstateAddress.isEmpty() || realEstateAddress == null) {
				throw new Exception();
			...
			RealEstateSpecifications spec3 = new RealEstateSpecifications(
					new SearchCriteria("realEstateSource", ":=", realEstateSource));
			Page<RealEstate> tempPage = realEstateRepository
					.findAll(Specification.where(Specification.where(spec1).or(spec2)).and(spec3), page);
			List<RealEstate> listTemp = tempPage.stream().map(x -> x).collect(Collectors.toList());
			if (listTemp.isEmpty() || listTemp == null) {
				throw new Exception("list data is empty");
			} else
	...
	@Override
	public Page<RealEstate> listFilterRealEstateByAddressAndSourceNot(String realEstateAddress, String realEstateSource,
			Pageable page) {
		try {
	...
	public long countByRealEstateAddress(String address) {
		try {
			...
			RealEstateSpecifications spec1 = new RealEstateSpecifications(
					new SearchCriteria("realEstateName", ":", address));
			RealEstateSpecifications spec2 = new RealEstateSpecifications(
					new SearchCriteria("realEstateAddress", ":", address));
...
//            }
			if (realEstate.getListHouse() != null && !realEstate.getListHouse().isEmpty()) {
	...
	public Page<RealEstate> findAllBySourceAndStatus(String source, String status, Pageable page) {
		try {
			List<RealEstateSpecifications> listSpec = new ArrayList();
			if (source != null) {
				RealEstateSpecifications spec = new RealEstateSpecifications(
						new SearchCriteria("realEstateSource", ":=", source));
				listSpec.add(spec);
			}
			if (status != null) {
				RealEstateSpecifications spec = new RealEstateSpecifications(
						new SearchCriteria("realEstateStatus", ":=", status));
				listSpec.add(spec);
			}
			if (listSpec.size() == 1) {
	...
	public Page<RealEstate> findAllByAttribute(Map<String, Object> listAttribute, Pageable page) {
		try {
			...
			if (!listSpec.isEmpty()) {
				Page<RealEstate> temp = realEstateRepository.findAll(createSpecification(listSpec), page);
				if(temp == null) {
					throw new Exception();
				}
				return temp;
